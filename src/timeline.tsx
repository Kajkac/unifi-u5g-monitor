...
export function TimelineView({ events }: { events: EventRow[] }) {
  ...
  return <div className="view"><section className="card timeline-card"><header><div><Activity size={18} /><h2>Operational timeline</h2></div><div className="filter-tabs">{groups.map((group) => <button key={group} className={filter === group ? 'active' : ''} onClick={() => setFilter(group)}>{group}</button>)}</div></header><div className="timeline">{rows.map((event) => <button className={`timeline-row ${event.level}`} key={`${event.id}-${event.ts}`} onClick={() => setSelected(event)}><div className="timeline-icon">{icon(event.kind)}</div><div><span>{event.kind}</span><strong>{event.message}</strong><time>{new Date(event.ts).toLocaleString()}</time></div></button>)}{rows.length === 0 && <div className="empty tall-empty"><Activity size={30} /><strong>No matching events</strong></div>}</div></section>{selected && <div className="modal-backdrop" onClick={() => setSelected(null)}><section className="modal event-modal" onClick={(e) => e.stopPropagation()}><header><div><span className="eyebrow">{selected.kind}</span><h2>{selected.message}</h2></div><button className="icon-btn" onClick={() => setSelected(null)}>×</button></header><div className={`notice ${selected.level === 'error' ? 'danger' : selected.level === 'warn' ? 'warn' : 'ok'}`}><ShieldAlert size={18} /><span>{selected.level.toUpperCase()} · {new Date(selected.ts).toLocaleString()}</span></div>{selected.data != null && <pre className="json-block">{JSON.stringify(selected.data, null, 2)}</pre>}</section></div>}</div>
}
