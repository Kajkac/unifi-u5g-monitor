...
import { Archive, Check, CheckCheck, Clock3, FileText, Inbox, MessageSquare, Plus, RefreshCw, Search, Send, Trash2, XCircle } from 'lucide-react'
...
export function SmsView({ status, viewer, notify }: { status: Status | null; viewer: boolean; notify: (tone: string, text: string) => void }) {
  ...
  return <div className="view sms-view">
    <section className="sms-shell">
      ...
      <div className="sms-main"><header className="sms-toolbar"><div><h2>{folders.find((item) => item.id === folder)?.label}</h2><span>{counts.unread} unread · {counts.failed} attention</span></div><div className="search"><Search size={15} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search messages" /></div><button className="icon-btn" onClick={load}><RefreshCw size={17} className={loading ? 'spin' : ''} /></button></header>
